...
class User(BaseMixin, ReprMixin, UserMixin, db.Model):
    ...
    confirmed_at = db.Column(db.DateTime(), server_default = func.current_date())
...
class Tasks(BaseMixin, ReprMixin, db.Model):
    ...
    owner_email = db.Column(db.ForeignKey("user.email", ondelete = "CASCADE"), nullable = False)
    task_name = db.Column(db.String(120), nullable = False)

    users = db.relationship('User', foreign_keys = [owner_email])
